...
You can download the pre-compiled binaries from the [Releases](https://github.com/Thought-Engineers/sil/releases/latest) page, or build it from source.

### Build Go Version
Ensure you have [Go](https://golang.org/doc/install) installed.

```bash
git clone https://github.com/Thought-Engineers/sil.git
cd sil
go build -o sil cmd/sil/main.go
```

### Build Rust Version (`rsil`)
The Rust version (`rsil`) is managed as a Git submodule hosted at [Thought-Engineers/rsil](https://github.com/Thought-Engineers/rsil). Ensure you have [Rust](https://rustup.rs/) installed.

```bash
git clone --recursive https://github.com/Thought-Engineers/sil.git
cd sil/rsil
cargo build --release
